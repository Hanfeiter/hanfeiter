...
## 1. Создал и заполнил 4 таблицы (Waste_Type/Operator/Collection_Poit/Waste_Remove_Act)
...
## 1.1 Запрос SELECT: Для 4-ёх таблиц
...
## 2. Выходные документы. 1-й документ (по заданию):
📊 Выдать отчет по общему весу вывезенных отходов каждого типа за месяц по районам города, отсортированный по убыванию веса.
![](pngFILE/select1.PNG)
![](pngFILE/select11.PNG)

## 3. Выходные документы. 2-й документ:
📅 Для заданной контейнерной площадки выдать график и частоту вывоза каждого типа отходов, отсортированный по дате.
![](pngFILE/select22.PNG)
...
## 1. Представления и процедуры
### Создано VIEW для запроса
![](3-4/prostgrafikvivoz.PNG)
### 1.1Создана функция simple_removal_schedule (Простой график вывоза)
![](3-4/grafikvivoz1.PNG)
**Проверка работы представления**

### 1.2 Создана функция system.simple_waste_report (Простой отчет по отходам за месяц)
![](3-4/prostoyotchet.PNG)

**Проверка работы функции**
![](3-4/prostoyotchet1.PNG)

### 2. Создание процедур для редактирования 

### 2.1 Создана процедура get_report_for_month (Получить отчет за месяц)
![](3-4/proc1.PNG)

### 2.2 Создана процедура Получить график для площадки (Получить отчет за месяц)
![](3-4/proc1.PNG)
...
## Генератор для таблицы *Waste_Types*
![](3-4/genwaste.PNG)
...
## Генератор для таблицы *Operator*
...
## Генератор для таблицы *Collection_Points*

![](3-4/gencollecton.PNG)

## **Проверка работы генератора**
![](3-4/gencollection1.PNG)

## Генератор для таблицы *Waste_Remove_Acts*

![](3-4/genwasteremove.PNG)

## **Проверка работы генератора**
![](3-4/genwasteremove1.PNG)

## Проведем анализ планов выполнения запроса 4(Waste_Remove_Acts)
![](3-4/analiz.PNG)
![](3-4/analiz1.PNG)
...
![](3-4/analiz3.PNG)
### До оптимизации время выполнения запроса состовляло 8.787 мc, происходило сканирование всей стаблицы из 20000 строк.
### После оптимизации выполнение запроса происходит по индексу и составляет 5.018 мс, что в 1,75 раз быстрее.
...
### При удалении оператора будем удалять все связанные акты вывоза отходов
# 1. Создан триггер функция delete_operator_cascade
![](5/5.1.PNG)
# 2. Создадим триггер на таблице Operator
![](5/5.2.PNG)
# 3. Проверка работы триггера
Выведем оператора, удалим его и проверим, есть ли информация об этом операторе в таблице Waste_Removal_Act.
![](5/5.3.PNG)
# 4. Создана таблица-журнал для отслеживания изменений в таблице Waste_Removal_Act
![](5/5.4.PNG)
# 5. Созданы триггеры аудита изменений (INSERT, UPDATE, DELETE)
# 5.1. Триггерная функция для аудита
![](5/5.8.PNG)
![](5/5.9.PNG)
![](5/5.10.PNG)
# 5.2. Триггер INSERT
